...
```bash
docker run -v $(pwd)/output:/output researchdeezer/spleeter separate -i audio_example.mp3 -o /output
```
...
nvidia-docker run -v $(pwd)/output:/output researchdeezer/spleeter:gpu separate -i audio_example.mp3 -o /output

# Or if your docker client version is high enough to support `Nvidia` runtime :
docker run --runtime=nvidia -v $(pwd)/output:/output researchdeezer/spleeter:gpu separate -i audio_example.mp3 -o /output
```
